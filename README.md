...
3. Gateway forwards the request to the upstream `/chat/completions` API with `stream: true`
4. As upstream tokens arrive, the **Reasoning Detector** classifies them as reasoning or output
5. Gateway streams to the client in three ordered phases:
   - **Phase 1: Prompt Summary** – streamed from the parallel summariser (minimal TTFT)
   - **Phase 2: Reasoning Summary** – generated after all reasoning tokens are collected
   - **Phase 3: Output** – the model's actual response tokens
...
- **If no reasoning is detected, Phase 2 is skipped.** The gateway degrades gracefully to just Phase 1 + Phase 3.
...
The gateway emits standard OpenAI-compatible SSE chunks, extended with `event: phase` markers:

```
event: phase
data: {"phase": "prompt_summary", "label": "Prompt Summary"}

data: {"id":"chatcmpl-gw-abc123","object":"chat.completion.chunk","choices":[{"index":0,"delta":{"content":"User asks about..."},"finish_reason":null}]}

event: phase
data: {"phase": "reasoning_summary", "label": "Reasoning Summary"}

data: {"id":"chatcmpl-gw-abc123","object":"chat.completion.chunk","choices":[{"index":0,"delta":{"content":"Model considered..."},"finish_reason":null}]}

event: phase
data: {"phase": "output", "label": "Response"}

data: {"id":"chatcmpl-gw-abc123","object":"chat.completion.chunk","choices":[{"index":0,"delta":{"content":"The answer is..."},"finish_reason":null}]}

data: [DONE]
```

**Compatibility:** Standard OpenAI clients that ignore `event:` lines will simply see all content concatenated. Phase-aware clients can parse the `event: phase` markers to render sections separately.
...
2. **Session pre-warming** – For apps with known system prompts, pre-computing the summary and warming the KV cache before the user submits eliminates Phase 1 latency entirely. The session API is intentionally simple (create → use → auto-expire) to minimize integration overhead.
